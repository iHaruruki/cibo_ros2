...
public:
  FrontCameraNode() : Node("front_camera") {
    ...
    image_sub_ = create_subscription<sensor_msgs::msg::Image>(
        "/camera_01/color/image_raw", 10,
        std::bind(&FrontCameraNode::image_callback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(), "Front Camera Node initialized with refined face mesh (478 landmarks + iris)");
  }
...
private:
  ...
  static void mouse_callback_static(int event, int x, int y, int flags, void* userdata) {
    auto* self = static_cast<FrontCameraNode*>(userdata);
  ...
  void process_image(const cv::Mat& cv_image, cv::Mat& annotated_image,
                     std::vector<float>& pose_landmarks, std::vector<float>& face_landmarks,
  ...
  void image_callback(const sensor_msgs::msg::Image::SharedPtr msg) {
    try {
      ...
      process_image(cv_image, annotated_image, pose_landmarks, face_landmarks,
      ...
      cv::putText(display_image, std::string("Face landmarks: ") + std::to_string(face_landmarks.size() / 3) + " points",
      ...
      try {
        ...
        auto face_msg = std_msgs::msg::Float32MultiArray();
        face_msg.data = face_landmarks;
        face_landmarks_pub_->publish(face_msg);
