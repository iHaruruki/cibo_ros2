...
class FrontCameraDepthNode : public rclcpp::Node {
public:
  FrontCameraDepthNode() : Node("front_camera"), tf_broadcaster_(this) {
    // Declare parameters
    ...
    declare_parameter("color_topic", "/front_camera/color/image_raw/compressed");
    declare_parameter("color_info_topic", "/front_camera/color/camera_info");
    declare_parameter("depth_topic", "/front_camera/depth/image_raw/compressedDepth");
    declare_parameter("depth_info_topic", "/front_camera/depth/camera_info");
    declare_parameter("camera_frame", "front_camera_depth_optical_frame");
    declare_parameter("publish_face_tf", false);
    ...
    publish_face_tf_ = get_parameter("publish_face_tf").as_bool();
    ...
    sync_->registerCallback(std::bind(&FrontCameraDepthNode::synced_callback, this,
                                       std::placeholders::_1, std::placeholders::_2,
                                       std::placeholders::_3));

    // Publishers
    annotated_pub_ =
        create_publisher<sensor_msgs::msg::Image>("/front_camera/annotated_image", 10);
    pose_landmarks_pub_ =
        create_publisher<std_msgs::msg::Float32MultiArray>("/front_camera/pose_landmarks", 10);
    face_landmarks_pub_ =
        create_publisher<std_msgs::msg::Float32MultiArray>("/front_camera/face_landmarks", 10);
    left_hand_landmarks_pub_ =
        create_publisher<std_msgs::msg::Float32MultiArray>("/front_camera/left_hand_landmarks", 10);
    right_hand_landmarks_pub_ =
        create_publisher<std_msgs::msg::Float32MultiArray>("/front_camera/right_hand_landmarks", 10);

    last_tf_time_ = now();

    RCLCPP_INFO(get_logger(), "Front Camera Depth Node initialized (holistic + face detection with depth→3D & tf broadcasting)");
  }

private:
  void setup_opencv_window() {
    try {
      cv::namedWindow("Front Camera - ROI Selection", cv::WINDOW_AUTOSIZE);
      cv::setMouseCallback("Front Camera - ROI Selection", mouse_callback_static, this);
  ...
  static void mouse_callback_static(int event, int x, int y, int flags, void* userdata) {
    auto* self = static_cast<FrontCameraDepthNode*>(userdata);
  ...
  void synced_callback(const CompressedImageMsg::SharedPtr color_msg,
    ...
    try {
      ...
      std::vector<float> pose_lm;
      std::vector<float> face_lm;
      ...
      // Publish landmarks
      publish_array(pose_landmarks_pub_, pose_lm);
      publish_array(face_landmarks_pub_, face_lm);
      ...
      cv::putText(display_image, std::string("Face landmarks: ") + std::to_string(face_lm.size() / 3) + " points",
                  cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 255, 0), 2);

      cv::imshow("Front Camera - ROI Selection", display_image);
  ...
  rclcpp::Publisher<std_msgs::msg::Float32MultiArray>::SharedPtr face_landmarks_pub_;
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::spin(std::make_shared<FrontCameraDepthNode>());
  rclcpp::shutdown();
  cv::destroyAllWindows();
  return 0;
}
